...
func testRecipeToManyRecipeidComments(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.RecipeidComments().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := RecipeSlice{&a}
	if err = a.L.LoadRecipeidComments(ctx, tx, false, (*[]*Recipe)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.RecipeidComments); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.RecipeidComments = nil
	if err = a.L.LoadRecipeidComments(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.RecipeidComments); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testRecipeToManyAddOpRecipeidComments(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddRecipeidComments(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.RecipeidComments[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.RecipeidComments[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.RecipeidComments().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testRecipeToManySetOpRecipeidComments(t *testing.T) {
	var err error
	...
	err = a.SetRecipeidComments(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetRecipeidComments(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.RecipeidComments[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.RecipeidComments[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testRecipeToManyRemoveOpRecipeidComments(t *testing.T) {
	var err error
	...
	err = a.AddRecipeidComments(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveRecipeidComments(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.RecipeidComments) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.RecipeidComments[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.RecipeidComments[0] != &e {
